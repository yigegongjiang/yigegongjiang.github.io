...
2. 虚拟网卡。操作系统会提供网络协议栈里面 **IP 网络层**数据包的 AOP 切面，是的软件层面被动监听到所有的 IP 层数据包。
3. socket hook。操作系统会提供数据包 hook 的能力，和 AOP 切面不同，hook 的影响未知，会对系统服务有破坏性。

Surge 使用了三种途径里面的两种，即系统代理和虚拟网卡。
...
很多成熟的大型应用，也都对 MitM 有校验。比如在进行证书验证的时候，明文判断证书是否符合预期这样，这时候 MitM 就会失败了。
