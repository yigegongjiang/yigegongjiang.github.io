...
对函数调用栈有更深了解的，可以查阅：[函数调用栈之彻底理解](https://www.yigegongjiang.com/2022/函数调用栈之彻底理解/)
...
**所以 fork 为什么有两次返回？其实根本不是 fork 函数有两次返回，没有一个函数能够返回两次，这违背了函数调用栈的原理**。详见：[函数调用栈之彻底理解](https://www.yigegongjiang.com/2022/函数调用栈之彻底理解/)
